...
function proxy(method, event) {
    return function callMethod(/*..args*/) {
        ...
        return new LiePromise(function(resolve, reject) {
            ...
            if (self.delayedSubscriptions) {
                var list = self.delayedSubscriptions;
                self.delayedSubscriptions = [];
                for (var item in list) {
                    var subscription = list[item];
                    api.subscribe(subscription.callback, subscription.event, subscription.context);
                }
            }
...
VPAID.prototype.subscribe = function subscribe(callback, event, context) {
    if (this.api) {
        this.api.subscribe(callback, event, context);
    } else {
        this.delayedSubscriptions.push({'event': event, 'callback': callback, 'context': context});
    }
};

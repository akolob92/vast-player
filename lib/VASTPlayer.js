...
function methodNotFoundError(name) {
	return new Error('Method "' + (name || '???') + '" not found.');
}
...
function proxy(method) {
	return function callMethod() {
		...
		if (player.api) {
			fn = player.api[method];
		} else {
			fn = player[method];
		}

		if (typeof fn !== 'function') {
			return LiePromise.reject(methodNotFoundError(method));
		}

		return LiePromise.resolve(fn.apply(player, arguments)).then(function () {
			return self;
		}).catch(function (err) {
			return LiePromise.reject(err);
		});
	};
}
...
VASTPlayer.prototype.getVpaid = function getVpaid() {
	var self = this;
	if (self.__private__.player instanceof JavaScriptVPAIDPlayer) {
		return self.__private__.player;
	} else {
		return null;
	}
};

function loadPlayer(vast, opts) {
	var self = this;
	...
	self.__private__.cleanCallback = opts && opts.cleanCallback;

	self.readyPromise = new LiePromise(function (resolve) {
		var intervalId = setInterval(function () {
			var _player = self.__private__.player;
			if (_player.video || _player.videoSlot) {
				resolve(self);
	...
	var isHTML = player instanceof HTMLVideoPlayer;

	if (!isHTML) {
		reporter.track(player);
	}

	return player.load(mediaFiles, parameters, opts).then(function () {
		if (isHTML) {
			reporter.track(player);
		}
	});
...
VASTPlayer.prototype.load = function load(uri, opts) {
	var self = this;
	var config = this.config.vast;

	return VAST.fetch(uri, config).then(function (vast) { return loadPlayer.call(self, vast, opts); }).then(function setReady() {
...
VASTPlayer.prototype.loadXml = function load(xml, opts) {
	var self = this;

	return LiePromise
		.resolve(new VAST(VAST.pojoFromXML(xml)))
		.then(function (vast) {
			var cfg = self.__private__ && self.__private__.config && self.__private__.config.vast || {};
			return cfg.resolveWrappers ? vast.resolveWrappers(cfg.maxRedirects) : vast;
		})
		.then(function (vast) {
			return loadPlayer.call(self, vast, opts);
		})
...
VASTPlayer.prototype.mute = proxy('mute');

VASTPlayer.prototype.unmute = proxy('unmute');

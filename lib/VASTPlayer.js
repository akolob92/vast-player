...
VASTPlayer.prototype.load = function load(uri) {
    ...
    return VAST.fetch(uri, config).then(loadPlayer.bind(this)).then(function setReady() {
...
VASTPlayer.prototype.loadXml = function load(xml) {
    ...
    return LiePromise.resolve(new VAST(VAST.pojoFromXML(xml))).then(loadPlayer.bind(this)).then(function setReady() {
        self.__private__.ready = true;
        self.emit('ready');
